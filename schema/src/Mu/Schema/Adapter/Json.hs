...
instance SLess.ToSchemalessTerm Value where
  toSchemalessTerm (Object o) =
    SLess.TRecord $
    map (\(k, v) -> SLess.Field k (SLess.toSchemalessValue v)) $ HM.toList o
  toSchemalessTerm v = SLess.TSimple (SLess.toSchemalessValue v)

instance SLess.ToSchemalessValue Value where
  toSchemalessValue r@(Object _) = SLess.FSchematic (SLess.toSchemalessTerm r)
...
instance ToJSONFields sch args =>
         ToJSON (Term sch ('DRecord name anns args)) where
  toJSON (TRecord fields) = Object (toJSONFields fields)

instance FromJSONFields sch args =>
         FromJSON (Term sch ('DRecord name anns args)) where
...
instance ToJSONEnum choices =>
         ToJSON (Term sch ('DEnum name anns choices)) where
  toJSON (TEnum choice) = String (toJSONEnum choice)

instance FromJSONEnum choices =>
         FromJSON (Term sch ('DEnum name anns choices)) where
...
instance ToJSON (FieldValue sch t) => ToJSON (FieldValue sch ('TOption t)) where
  toJSON (FOption v) = toJSON v

instance ToJSON (FieldValue sch t) => ToJSON (FieldValue sch ('TList t)) where
...
instance ( FromJSONKey (FieldValue sch k)
         , FromJSON (FieldValue sch v)
