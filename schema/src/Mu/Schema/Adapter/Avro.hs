...
instance SLess.ToSchemalessTerm (AVal.Value t) where
  toSchemalessTerm (AVal.Record _ r) =
    SLess.TRecord $
    map (\(k, v) -> SLess.Field k (SLess.toSchemalessValue v)) $ HM.toList r
  toSchemalessTerm (AVal.Enum _ i _) = SLess.TEnum i
  toSchemalessTerm (AVal.Union _ _ v) = SLess.toSchemalessTerm v
...
instance SLess.ToSchemalessValue (AVal.Value t) where
  ...
  toSchemalessValue (AVal.Map hm) =
    SLess.FMap $
    M.fromList $
    map (SLess.FPrimitive *** SLess.toSchemalessValue) $ HM.toList hm
  toSchemalessValue (AVal.Union _ _ v) = SLess.toSchemalessValue v
  toSchemalessValue r@(AVal.Record _ _) =
    SLess.FSchematic (SLess.toSchemalessTerm r)
  toSchemalessValue e@AVal.Enum {} = SLess.FSchematic (SLess.toSchemalessTerm e)

instance HasAvroSchemas sch sch => A.HasAvroSchema (WithSchema sch sty t)
...
instance forall r d ds. (A.HasAvroSchema (Term r d), HasAvroSchemas r ds) =>
         HasAvroSchemas r (d ': ds) where
  schemas pr _ = V.cons thisSchema (schemas pr (Proxy @ds))
    where
      thisSchema = unTagged $ A.schema @(Term r d)
...
instance (KnownName name, HasAvroSchemaFields sch args) =>
  ...
  schema = Tagged $ ASch.Record recordName [] Nothing Nothing fields
    where
      recordName = nameTypeName (Proxy @name)
...
instance (KnownName name, HasAvroSchemaEnum choices) =>
  ...
  schema = Tagged $ ASch.mkEnum enumName [] Nothing choicesNames
    where
      enumName = nameTypeName (Proxy @name)
...
instance KnownName t => A.HasAvroSchema (FieldValue sch ('TSchematic t))
  -- schema = coerce $ A.schema @(Term sch (sch :/: t))
                                                        where
  schema = Tagged $ ASch.NamedType (nameTypeName (Proxy @t))

instance forall sch choices. HasAvroSchemaUnion (FieldValue sch) choices =>
         A.HasAvroSchema (FieldValue sch ('TUnion choices)) where
  schema =
    Tagged $ ASch.mkUnion $ schemaU (Proxy @(FieldValue sch)) (Proxy @choices)
...
instance ( HasAvroSchemaUnion (FieldValue sch) choices
...
instance (A.FromAvro (FieldValue sch u), FromAvroUnion sch us) =>
  ...
  fromAvroU branch v
    | ASch.matches branch (unTagged (A.schema @(FieldValue sch u))) =
      Z <$> A.fromAvro v
    | otherwise = S <$> fromAvroU branch v

class FromAvroFields sch (fs :: [FieldDef Symbol Symbol]) where
  fromAvroF ::
       HM.HashMap T.Text (AVal.Value ASch.Type) -> A.Result (NP (Field sch) fs)
...
instance (KnownName name, HasAvroSchemaFields sch args, ToAvroFields sch args) =>
  ...
  toAvro (TRecord fields) = AVal.Record wholeSchema (toAvroF fields)
    where
      wholeSchema = unTagged (A.schema @(Term sch ('DRecord name anns args)))
...
instance (KnownName name, HasAvroSchemaEnum choices, ToAvroEnum choices) =>
  ...
  toAvro (TEnum n) = AVal.Enum wholeSchema choice text
    where
      wholeSchema = unTagged (A.schema @(Term sch ('DEnum name anns choices)))
      (choice, text) = toAvroE n

instance A.ToAvro (FieldValue sch t) => A.ToAvro (Term sch ('DSimple t)) where
...
instance forall sch choices. ( HasAvroSchemaUnion (FieldValue sch) choices
  ...
  toAvro (FUnion v) = AVal.Union wholeSchema' chosenTy chosenVal
    where
      wholeSchema = schemaU (Proxy @(FieldValue sch)) (Proxy @choices)
...
instance forall sch u us. (A.ToAvro (FieldValue sch u), ToAvroUnion sch us) =>
